...
export function getModuleInstanceState(node: ModuleDeclaration, visited?: Map<number, ModuleInstanceState | undefined>): ModuleInstanceState {
    if (node.body && !node.body.parent) {
        // getmoduleinstancestateforaliastarget precisa percorrer a cadeia parent, então
        // os ponteiros parents devem estar definidos nesta árvore já
...
function getModuleInstanceStateWorker(node: Node, visited: Map<number, ModuleInstanceState | undefined>): ModuleInstanceState {
    // um módulo não é instanciado se ele contém apenas
    switch (node.kind) {
        // 1. declarações de interface, declarações de alias de tipo
        case SyntaxKind.InterfaceDeclaration:
        case SyntaxKind.TypeAliasDeclaration:
            return ModuleInstanceState.NonInstantiated;

        // 2. declarações de enumeração const
        ...
        // 3. declarações de importação não exportadas
        ...
        // 4. exportar declarações de alias apontando apenas para módulos não instanciados ou coisas que os módulos não instanciados contêm
        ...
        // 5. outras declarações de módulos não instanciados
            ...
            forEachChild(node, n => {
                ...
                switch (childState) {
                    case ModuleInstanceState.NonInstantiated:
                        // children não é instanciada - continuar procurando

                        return;

                    case ModuleInstanceState.ConstEnumOnly:
                        // children é const enum apenas - registrar o estado e continuar pesquisando
                    ...
                    case ModuleInstanceState.Instantiated:
                        // a children é instanciada - registrar o estado e parar
        ...
        case SyntaxKind.Identifier:
            // somente a definição typedef jsdoc pode existir no namespace jsdoc e
            // deve ser considerada igual ao alias de tipo
...
function getModuleInstanceStateForAliasTarget(specifier: ExportSpecifier, visited: Map<number, ModuleInstanceState | undefined>) {
    ...
    if (name.kind !== SyntaxKind.Identifier) {
        return ModuleInstanceState.Instantiated; // ignorar sintaxe inválida como esta: export { "x" }
    }
            ...
            for (const statement of statements) {
                if (nodeHasName(statement, name)) {
                    ...
                    if (statement.kind === SyntaxKind.ImportEqualsDeclaration) {
                        // tratar reexportações de aliases de importação como instanciadas, pois
                        // são ambíguas. isso é consistente com `export import x = mod.x` sendo
                        // tratado como instanciado:
                        //
                        // import x = mod.x;
                        // export { x };
    ...
    return ModuleInstanceState.Instantiated; // não foi possível localizar, presumir que possa se referir a um valor
}

/** @internal */
export const enum ContainerFlags {
    // o node atual não é um contêiner e nenhuma manipulação de contêiner deve ocorrer antes de recorrer a ele
    None = 0,

    // o node atual é um contêiner. ele deve ser definido como o contêiner atual (e contêiner de bloco) antes
    // de recorrer a ele. o node atual não possui locais. examplos:
    //
    // classes, objectliterals, typeliterals, interfaces...
    IsContainer = 1 << 0,

    // o node atual é um contêiner com escopo de bloco. ele deve ser definido como o contêiner de bloco atual
    // antes de executar uma recursão nele. examplos:
    //
    // blocos (quando não é parente de funções), cláusulas de captura, instruções for/for-in/for-of...
    IsBlockScopedContainer = 1 << 1,

    // o node atual é o contêiner de um caminho de fluxo de controle. o fluxo de controle atual deve ser
    // salvo e restaurado, e um novo fluxo de controle deve ser inicializado dentro do contêiner
...
function createBinder(): (file: SourceFile, options: CompilerOptions) => void {
    // por que var? ele evita verificações de tdz no tempo de execução, o que pode ser caro
    //
    // veja: https://github.com/microsoft/typescript/issues/52924
    ...
    var thisParentContainer: IsContainer | EntityNameExpression; // contêiner um nível acima
    ...
    // estado usado pela análise de fluxo de controle
    ...
    // estado usado para emitir ajudantes
    var emitFlags: NodeFlags;

    // se este arquivo for um módulo externo, ele estará automaticamente no modo estrito
    // de acordo com o es6. se não for um módulo externo, determinaremos se ele está no
    // modo estrito ou não, dependendo se vemos "use strict" em certos lugares ou se
    // atingimos uma classe/namespace ou se as opções do compilador contêm alwaysstrict
    var inStrictMode: boolean;

    // se estivermos vinculando um padrão de atribuição, vincularemos certas expressões
    // de forma diferente
    ...
    var unreachableFlow = createFlowNode(FlowFlags.Unreachable, /* node */ undefined, /* antecedente */ undefined);
    var reportedUnreachableFlow = createFlowNode(FlowFlags.Unreachable, /* node */ undefined, /* antecedente */ undefined);
    ...
    /**
     * dentro do binder, podemos criar um diagnóstico para um node ainda não vinculado (potencialmente sem ponteiros
     * parent, o que implica nenhum arquivo de origem acessível)
     *
     * se for assim, o node deve estar no arquivo atual (pois essa é a única maneira de algo ter passado por
     * ele e gerá-lo como o node de erro)
     *
     * essa versão de `creatediagnosticfornode` usa o contexto do binder para levar isso em conta e sempre produz
     * diagnósticos corretos, mesmo nessas situações
     */
    ...
    function bindSourceFile(f: SourceFile, opts: CompilerOptions) {
        ...
        // anexar informações de depuração, se necessário
    ...
    function bindInStrictMode(file: SourceFile, opts: CompilerOptions): boolean {
        if (getStrictOptionValue(opts, "alwaysStrict") && !file.isDeclarationFile) {
            // vincular em modo estrito arquivos de origem com a opção alwaysstrict
    ...
    function addDeclarationToSymbol(symbol: Symbol, node: Declaration, symbolFlags: SymbolFlags) {
        ...
        // na fusão do módulo const enum com classe ou função, redefina o sinalizador const enum only (os namespaces já serão recalculados)
    ...
    // não deve ser chamado em uma declaração com um nome de propriedade computado, a menos
    // que seja um símbolo bem conhecido
    function getDeclarationName(node: Declaration): __String | undefined {
        ...
        if (name) {
            ...
            if (name.kind === SyntaxKind.ComputedPropertyName) {
                ...
                // tratar nomes de propriedades computadas onde a expressão é uma
                // string/literal numérica como apenas string/literal numérica
                ...
                if (isSignedNumericLiteral(nameExpression)) {
                    ...
                    Debug.fail("somente propriedades computadas com nomes literais têm nomes de declaração");
                }
            }

            if (isPrivateIdentifier(name)) {
                // containingclass existe porque nomes privados são permitidos somente dentro de classes
                const containingClass = getContainingClass(node);

                if (!containingClass) {
                    // podemos chegar aqui em casos onde já há um erro de análise
        ...
        switch (node.kind) {
            ...
            case SyntaxKind.SourceFile:
                // o arquivo json deve se comportar como:
                //
                // module.exports = ...
            ...
            case SyntaxKind.BinaryExpression:
                ...
                Debug.fail("tipo de declaração binária desconhecida");
            ...
            case SyntaxKind.Parameter:
                // parâmetros com nomes são manipulados no topo desta função. parâmetros
                // sem nomes só podem vir de jsdocfunctiontypes

                Debug.assert(node.parent.kind === SyntaxKind.JSDocFunctionType, "tipo de parâmetro parent impossível", () => `parent é: ${Debug.formatSyntaxKind(node.parent.kind)}, jsdocfunctiontype esperado`);
    ...
    /**
     * declara um símbolo para o node e o adiciona aos símbolos. relata erros
     * para nomes de identificadores conflitantes
     *
     * @param symbolTable - a tabela de símbolos à qual o node será adicionado
     * @param parent - declaração do parent do node
     * @param node - a declaração a ser adicionada à tabela de símbolos
     * @param includes - os symbolflags que o node possui, além do seu tipo de declaração (por exemplo: export, ambient, etc.)
     * @param excludes - os sinalizadores cujo o node não pode ser declarado em uma tabela de símbolos. usado para relatar declarações proibidas
     */
    function declareSymbol(symbolTable: SymbolTable, parent: Symbol | undefined, node: Declaration, includes: SymbolFlags, excludes: SymbolFlags, isReplaceableByMethod?: boolean, isComputedName?: boolean): Symbol {
        ...
        // o símbolo exportado para um node de função/classe padrão de exportação é sempre denominado "padrão"
        ...
        if (name === undefined) {
            ...
            // verificar se a tabela de símbolos já possui um símbolo com este nome. caso
            // contrário, crie um novo símbolo com esse nome e adicione-o à tabela. notar
            // que não damos nenhum sinalizador ao novo símbolo *ainda*. isso garante
            // que não haverá conflito com os sinalizadores "excludes" que passamos
            //
            // se obtivermos um símbolo existente, vejamos se ele entra em conflito com o
            // novo símbolo que estamos criando. por exemplo, um símbolo 'var' e um
            // símbolo 'class' entrarão em conflito na mesma tabela de símbolos
            //
            // se tivermos um conflito, relate o problema em cada declaração que tivermos
            // para este símbolo e, em seguida, crie um novo símbolo para esta declaração
            //
            // observe que quando propriedades declaradas em construtores javascript
            // (marcadas por isreplaceablebymethod) entram em conflito com outro símbolo,
            // a propriedade perde. sempre
            //
            // isso permite o padrão comum do javascript de substituir um método de
            // protótipo por um método de instância vinculado do mesmo tipo:
            // `this.method = this.method.bind(this)`
            //
            // se criamos um novo símbolo, seja porque não tínhamos um símbolo com
            // esse nome na tabela de símbolos, ou porque houve conflito com um símbolo
            // existente, basta adicionar esse node como a única declaração do novo símbolo
            //
            // caso contrário, estaremos mesclando em um símbolo existente compatível
            // (por exemplo, quando você tem várias 'vars' com o mesmo nome no mesmo
            // contêiner). neste caso basta adicionar este node na lista de declarações
            // do símbolo
            ...
            if (!symbol) {
                ...
                // já existe um símbolo, então não adicionar isso como uma declaração

                return symbol;
            } else if (symbol.flags & excludes) {
                if (symbol.isReplaceableByMethod) {
                    // símbolos declarados pelo construtor javascript podem ser
                    // descartados em favor de símbolos de protótipo, como métodos

                    symbolTable.set(name, symbol = createSymbol(SymbolFlags.None, name));
                } else if (!(includes & SymbolFlags.Variable && symbol.flags & SymbolFlags.Assignment)) {
                    // declarações de atribuição podem ser mescladas com variáveis, não importando
                    // quais outros sinalizadores elas tenham

                    if (isNamedDeclaration(node)) {
                        setParent(node.name, node);
                    }

                    // relatar erros em todas as posições com declaração duplicada
                    //
                    // relatar erros em declarações encontradas anteriormente
                    ...
                    if (length(symbol.declarations)) {
                        // se o node atual for uma exportação padrão de algum tipo, verificar
                        // se há outras exportações padrão nas quais precisamos gerar erros
                        //
                        // saberemos se temos outras exportações padrão dependendo se
                        // `symbol` já tem uma lista de declaração definida
                        ...
                        if (isDefaultExport) {
                            ...
                            // isso serve para relatar corretamente um erro caso "export default { }" esteja
                            // após export default da declaração de classe ou função
                            //
                            // erro na exportação múltipla padrão no seguinte caso:
                            //
                            // 1. exportação múltipla padrão de declaração de classe ou declaração de função verificando nodeflags.default
                            //
                            // 2. exportação múltipla padrão da atribuição de exportação. esse não tem nodeflags.default ativado
                            // (já que a exportação padrão não é considerada como modificador)
                    ...
                    if (isTypeAliasDeclaration(node) && nodeIsMissing(node.type) && hasSyntacticModifier(node, ModifierFlags.Export) && symbol.flags & (SymbolFlags.Alias | SymbolFlags.Type | SymbolFlags.Namespace)) {
                        // export type T; - pode ter significado export type { T }?
        ...
        if (symbol.parent) {
            Debug.assert(symbol.parent === parent, "o símbolo parent existente deve corresponder ao novo");
        } else {
    ...
    function declareModuleMember(node: Declaration, symbolFlags: SymbolFlags, symbolExcludes: SymbolFlags): Symbol {
        ...
        if (symbolFlags & SymbolFlags.Alias) {
            ...
            // os membros do módulo exportados recebem 2 símbolos: um símbolo local que é classificado
            // com um sinalizador exportvalue e um símbolo de exportação associado com todos os
            // sinalizadores corretos definidos nele. há duas razões principais:
            //
            // 1. tratamos produtos locais e exportações de mesmo nome como mutuamente exclusivos dentro de um contêiner
            //     isso significa que o binder emitirá um erro de identificador duplicado se você misturar locais e
            //     exportações com o mesmo nome no mesmo contêiner
            //
            //     todo: tornar isso um erro mais específico e separe-o da lógica de exclusão
            //
            // 2. quando verificamos o identificador no verificador, definimos seu símbolo resolvido como o símbolo local,
            //     mas retorna o símbolo de exportação (por chamar getexportsymbolofvaluesymbolifexported). dessa forma,
            //     quando o emissor retornar, ele saberá que não deve qualificar o nome se ele for encontrado em um
            //     escopo de contenção
            //
            // nota: módulos de ambiente aninhados sempre devem ir para a tabela 'locais' para evitar sua mesclagem
            //     automática durante a mesclagem global no verificador. por quê? o único caso em que um módulo ambiente
            //     é permitido dentro de outro módulo é o de aumento de módulo, e este caso é tratado de forma especial.
            //     os aumentos de módulo devem ser mesclados somente com a definição original do módulo e nunca devem
            //     ser mesclados diretamente com outros aumentos, e o último caso seria possível se a mesclagem
            //     automática fosse permitida.

            if (isJSDocTypeAlias(node))
                Debug.assert(isInJSFile(node)); // não devemos adicionar símbolos para nodes jsdoc se não estiverem em um arquivo js
            ...
            if (!isAmbientModule(node) && (hasExportModifier || container.flags & NodeFlags.ExportContext)) {
                if (!canHaveLocals(container) || !container.locals || (hasSyntacticModifier(node, ModifierFlags.Default) && !getDeclarationName(node))) {
                    return declareSymbol(container.symbol.exports!, container.symbol, node, symbolFlags, symbolExcludes); // nenhum símbolo local para um padrão sem nome
                }
    ...
    function jsdocTreatAsExported(node: Node) {
        ...
        // o tratamento do typedef do jsdoc é um pouco complicado, mas para resumir, tratar o typedef como exportado se:
        //
        // 1. ele tem um nome explícito (já que por padrão os typedefs são sempre exportados diretamente, seja no
        // nível superior ou em um contêiner), ou

        if (!isJSDocEnumTag(node) && !!node.fullName)
            return true;

        // 2. a coisa de onde um typedef sem nome extrai seu nome é implicitamente uma exportação direta (por atribuição
        // ou sinalizador de exportação real)
        ...
        // isso poderia ser potencialmente simplificado se `delayedbindjsdoctypedeftag` passasse uma substituição
        // para `hasexportmodifier`, já que ele já deveria ter calculado e ramificado na maior parte disso

        return false;
    }

    // todos os nodes do contêiner são mantidos em uma lista vinculada em ordem de declaração. esta lista é usada
    // pela função getlocalnameofcontainer no verificador de tipos para validar se o nome local usado para um
    // contêiner é exclusivo

    function bindContainer(node: Mutable<HasContainerFlags>, containerFlags: ContainerFlags) {
        // antes de recorrermos aos childrens de um node, primeiro salvamos o parent, o contêiner
        // e o bloco-contêiner existentes
        //
        // depois de terminarmos de processar os childrens, restauramos esses valores salvos
        ...
        if (node.kind === SyntaxKind.ArrowFunction && node.body.kind !== SyntaxKind.Block)
            inReturnPosition = true;

        // dependendo do tipo de node, talvez seja necessário ajustar o contêiner atual e o
        // contêiner de bloco. se o node atual for um contêiner, ele também será
        // automaticamente considerado o contêiner de bloco atual. além disso, para contêineres
        // que sabemos que podem conter locais, inicializamos ansiosamente o campo .locals.
        // fazemos isso porque é muito provável que os .locals sejam necessários para colocar
        // algum children (por exemplo, um parâmetro ou declaração de variável).
        //
        ...
        if (containerFlags & ContainerFlags.IsControlFlowContainer) {
            ...
            // um iife não assíncrono e não gerador é considerado parte do fluxo de controle de contenção
            //
            // as instruções return se comportam de forma semelhante às instruções break que saem para
            // um rótulo logo após o corpo da instrução
            ...
            // criamos um gráfico de fluxo de controle de retorno para iifes e construtores
            //
            // para os construtores, usamos o gráfico de fluxo de controle de retorno em
            // verificações de inicialização de propriedade estritas
            ...
            // redefine todos os sinalizadores relacionados à verificação de acessibilidade no node (para cenários incrementais)
            ...
            Debug.assertNotNode(node, isIdentifier); // containsthis não pode se sobrepor a hasextendedinicodeescape no identificador
    ...
    function bindChildren(node: Node): void {
        ...
        // a maioria dos nodes não são válidos em um padrão de atribuição, então
        // limpamos o valor aqui e o definimos antes de avançarmos para os nodes
        // que podem realmente fazer parte de um padrão de atribuição
        ...
        switch (node.kind) {
            ...
            case SyntaxKind.BinaryExpression:
                if (isDestructuringAssignment(node)) {
                    // transferir se estamos em um padrão de atribuição para expressões
                    // binárias que poderiam realmente ser um inicializador
            ...
            // em arquivos de origem e blocos, vincular as funções primeiro para corresponder à elevação
            // que ocorre em tempo de execução
            ...
            case SyntaxKind.SpreadElement:
                // transferir se estamos em um padrão de atribuição de literais de objeto
                // e matriz, bem como seus childrens que são alvos de atribuição válidos
    ...
    function isNarrowableReference(expr: Expression): boolean {
        switch (expr.kind) {
            ...
            case SyntaxKind.ElementAccessExpression:
                return (isStringOrNumericLiteralLike((expr as ElementAccessExpression).argumentExpression) || isEntityNameExpression((expr as ElementAccessExpression).argumentExpression)) && isNarrowableReference((expr as ElementAccessExpression).expression);

            case SyntaxKind.BinaryExpression:
                return (expr as BinaryExpression).operatorToken.kind === SyntaxKind.CommaToken && isNarrowableReference((expr as BinaryExpression).right) || isAssignmentOperator((expr as BinaryExpression).operatorToken.kind) && isLeftHandSideExpression((expr as BinaryExpression).left);
        }
    ...
    function hasNarrowableArgument(expr: CallExpression) {
        ...
        if (expr.expression.kind === SyntaxKind.PropertyAccessExpression && containsNarrowableReference((expr.expression as PropertyAccessExpression).expression)) {
            return true;
        }
    ...
    function createBranchLabel() {
        return createFlowNode(FlowFlags.BranchLabel, /* node */ undefined, /* antecedente */ undefined) as FlowLabel;
    }

    function createLoopLabel() {
        return createFlowNode(FlowFlags.LoopLabel, /* node */ undefined, /* antecedente */ undefined) as FlowLabel;
    }
    ...
    function setFlowNodeReferenced(flow: FlowNode) {
        // na primeira referência, definimos a flag referenced e, em seguida, definimos a flag shared
    ...
    function createFlowCondition(flags: FlowFlags.TrueCondition | FlowFlags.FalseCondition, antecedent: FlowNode, expression: Expression | undefined) {
        ...
        if ((expression.kind === SyntaxKind.TrueKeyword && flags & FlowFlags.FalseCondition || expression.kind === SyntaxKind.FalseKeyword && flags & FlowFlags.TrueCondition) && !isExpressionOfOptionalChainRoot(expression) && !isNullishCoalesce(expression.parent)) {
    ...
    function bindTryStatement(node: TryStatement): void {
        // assumimos conservadoramente que *qualquer* código no bloco try pode causar uma exceção, mas
        // precisamos rastrear apenas o código que causa mutações (porque somente mutações ampliam o possível
        // tipo de fluxo de controle de uma variável)
        //
        // o exceptionlabel é o rótulo de destino para fluxos de controle resultantes de exceções
        //
        // adicionamos todos os nodes de fluxo de mutação como antecedentes desse rótulo para que possamos
        // analisá-los como possíveis antecedentes do início dos blocos catch ou finally. além disso,
        // adicionamos o fluxo de controle atual para representar exceções que ocorrem antes de quaisquer
        // mutações
    ...
    function declareSymbolAndAddToSymbolTable(node: Declaration, symbolFlags: SymbolFlags, symbolExcludes: SymbolFlags): Symbol | undefined {
        switch (container.kind) {
            // módulos, arquivos de origem e classes precisam de tratamento especializado sobre como seus membros são
            // declarados (por exemplo, um membro de uma classe entrará em uma tabela de símbolos específica, dependendo
            // se ela é estática ou não). nós repassamos a responsáveis ​​especializados a tarefa de declarar esses
            // membros childrens
            ...
            case SyntaxKind.MappedType:
                // todos os childrens desses tipos de contêineres nunca são visíveis por meio de outro símbolo (ou seja, por meio
                // de 'exportações' ou 'membros' de outro símbolo). em vez disso, eles são acessados ​​apenas 'lexicamente' (ou
                // seja, a partir do código que existe abaixo do contêiner na árvore). para fazer isso, simplesmente adicionamos
                // o símbolo declarado aos 'locais' do contêiner. esses símbolos podem ser encontrados à medida que o verificador
                // de tipos percorre os contêineres, verificando se há nomes correspondentes
    ...
    function setExportContextFlag(node: Mutable<ModuleDeclaration | SourceFile>) {
        // um arquivo de origem de declaração ou declaração de módulo de ambiente que não contém
        // declarações de exportação (mas possivelmente declarações regulares com modificadores
        // de exportação) é um contexto de exportação no qual as declarações são exportadas
        // implicitamente
    ...
    function bindModuleDeclaration(node: ModuleDeclaration) {
        ...
        if (isAmbientModule(node)) {
            ...
            if (state !== ModuleInstanceState.NonInstantiated) {
                ...
                // se o módulo já foi mesclado com alguma função, classe ou enumeração não constante, tratar como não constante-apenas-enumeração
                symbol.constEnumOnlyModule = (!(symbol.flags & (SymbolFlags.Function | SymbolFlags.Class | SymbolFlags.RegularEnum)))
                    // atual deve ser somente `const enum`
                    && state === ModuleInstanceState.ConstEnumOnly

                    // não pode ter sido definido como 'false' em um símbolo mesclado anteriormente ('undefined' ok)
                    && symbol.constEnumOnlyModule !== false;
            }
        }
    }
    ...
    function bindJSDocImports() {
        ...
        for (const jsDocImportTag of jsDocImports) {
            ...
            currentFlow = createFlowNode(FlowFlags.Start, /* node */ undefined, /* antecedente */ undefined);
    ...
    // o binder visita cada node na árvore de sintaxe, portanto é um local conveniente para executar
    // uma única verificação localizada de palavras reservadas usadas como identificadores no código
    // de modo estrito, bem como `yield` ou `await` em contextos [yield] ou [await], respectivamente

    function checkContextualIdentifier(node: Identifier) {
        // relatar erro somente se não houver erros de análise no arquivo
        if (
            ...
            // identificadores de modo estrito
    ...
    function bind(node: Node | undefined): void {
        ...
        if (tracing)
            (node as TracingNode).tracingPath = file.path;
    ...
    /// deve ser chamado apenas em diretivas de prólogo (isprologuedirective(node) deve ser true)
    function isUseStrictPrologueDirective(node: ExpressionStatement): boolean {
        const nodeText = getSourceTextOfNodeFromSourceFile(file, node.expression);

        // nota: o texto do node deve ser exatamente "use strict" ou 'use strict'. não é aceitável
        // que a string contenha escapes unicode (conforme es5)
    ...
    function bindWorker(node: Node) {
        switch (node.kind) {
            /* verificações de modo estrito */

            case SyntaxKind.Identifier:
                // para nomes de tipo typedef com namespaces, vincule o novo símbolo de tipo jsdoc aqui
                // porque ele requer que todos os namespaces contidos estejam em vigor, ou seja, o
                // "blockscopecontainer" atual precisa ser definido como seu namespace parent imediato
            ...
            case SyntaxKind.BinaryExpression:
                ...
                switch (specialKind) {
                    ...
                    default:
                        Debug.fail("tipo de atribuição de propriedade especial de expressão binária desconhecida");
                }
            ...
            case SyntaxKind.TypePredicate:
                break; // amarrar os childrens cuidará de tudo
            ...
            case SyntaxKind.MethodDeclaration:
            case SyntaxKind.MethodSignature:
                // se este for um método objectliteralexpression, ele ficará no mesmo espaço que
                // outras propriedades no literal do objeto. então usamos symbolflags.propertyexcludes
                // para que ele entre em conflito com quaisquer outros membros literais de objeto
                // com o mesmo nome
            ...
            case SyntaxKind.CallExpression:
                ...
                switch (assignmentKind) {
                    ...
                    default:
                        return Debug.fail("tipo de declaração de atribuição de expressão de chamada desconhecida");
                }
            ...
            // membros de classes, interfaces e módulos
            case SyntaxKind.ClassExpression:
            case SyntaxKind.ClassDeclaration:
                // todas as classes estão automaticamente no modo estrito no es6
    ...
    function bindSourceFileIfExternalModule() {
        ...
        if (isExternalModule(file)) {
            ...
            // criar um símbolo equivalente para o module.exports = {}
    ...
    /**
     * os valores expando do javascript são:
     *
     * - funções
     * - classes
     * - namespaces
     * - variáveis ​​inicializadas com expressões de função
     * -                         com expressões de classe
     * -                         com literais de objetos vazios
     * -                         com literais de objeto não vazios se atribuídos à propriedade de protótipo
     */
    function isExpandoSymbol(symbol: Symbol): boolean {
        ...
        let init = !node ? undefined :
            ...
            isPropertyAccessExpression(node) && isBinaryExpression(node.parent) ? node.parent.right : undefined;
    ...
    function bindClassLikeDeclaration(node: ClassLikeDeclaration) {
        if (node.kind === SyntaxKind.ClassDeclaration) {
            ...
            // adicionar nome de expressão de classe no mapa para classificador semântico
            if (node.name) {
        ...
        // typescript 1.0 spec (abril de 2014): 8.4
        //
    ...
    function checkUnreachable(node: Node): boolean {
        ...
        if (currentFlow === unreachableFlow) {
            const reportError =
                // relatar erro em todas as declarações, exceto as vazias
                (isStatementButNotDeclaration(node) && node.kind !== SyntaxKind.EmptyStatement) ||

                // relatar erro em declarações de classe
                node.kind === SyntaxKind.ClassDeclaration ||

                // relatar erros em enumerações com emissão preservada
                isEnumDeclarationWithPreservedEmit(node, options) ||

                // relatar erro em módulos instanciados
            ...
            if (reportError) {
                ...
                if (!options.allowUnreachableCode) {
                    // código inacessível é relatado se:
                    //
...
function eachUnreachableRange(node: Node, options: CompilerOptions, cb: (start: Node, last: Node) => void): void {
    ...
    // ao contrário de uma declaração pura como uma `interface`
    function isExecutableStatement(s: Statement): boolean {
        // não remover as declarações que podem ser usadas validamente antes que elas apareçam

        return !isFunctionDeclaration(s) && !isPurelyTypeDeclaration(s) &&
            // `var x;` pode declarar uma variável usada acima
...
export function getContainerFlags(node: Node): ContainerFlags {
    switch (node.kind) {
        ...
        case SyntaxKind.JSDocImportTag:
            // tratar como um contêiner para evitar o uso de um host efetivo envolvente, garantindo
            // que as vinculações de importação tenham o escopo definido corretamente
        ...
        case SyntaxKind.Block:
            // não tratar blocos diretamente dentro de uma função como um contêiner com escopo de bloco.
            //
