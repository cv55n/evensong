...
module.exports = createRule({
    ...
    meta: {
        docs: {
            description: `não permite o uso de certas palavras-chave do typescript como nomes de variáveis ou parâmetros`,
        },

        messages: {
            noKeywordsError: `{{ name }} conflita com palavra-chave/tipo`,
        },
    ...
    create(context) {
        ...
        return {
            VariableDeclarator(node) {
                ...
                if (node.id.type === AST_NODE_TYPES.Identifier && isKeyword(node.id.name)) {
                    report(node.id);
                }
            },
