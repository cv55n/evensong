...
module.exports = createRule({
    ...
    meta: {
        ...
        messages: {
            argumentTriviaArgumentError: `argumento de tag com nome de parâmetro`,
            argumentTriviaArgumentSpaceError: `deve haver 1 espaço entre um argumento e seu comentário`,
            argumentTriviaArgumentNameError: `o nome do argumento "{{ got }}" não corresponde ao nome esperado "{{ want }}"`
        },
    ...
    create(context) {
        ...
        const shouldIgnoreCalledExpression = node => {
            if (node.callee && node.callee.type === AST_NODE_TYPES.MemberExpression) {
                const methodName = node.callee.property.type === AST_NODE_TYPES.Identifier ? node.callee.property.name : "";
        ...
        const checkArg = (node, i, getSignature) => {
            ...
            const getExpectedName = memoize(() => {
                ...
                if (signature) {
                    ...
                    if (expectedName) {
                        ...
                        // se um parâmetro não for utilizado, acrescentamos um sublinhado. ignorar isso
                        // para que possamos alternar entre usados ​​e não usados ​​sem modificar o código,
                        // exigindo que os argumentos sejam marcados com o nome não sublinhado
            ...
            if (argRangeStart !== commentRangeEnd + 1 && !hasNewLine) {
                // todo: o alcance deve ser um espaço em branco
