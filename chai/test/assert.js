...
describe('assert', function() {
	...
	it('assert', function() {
		...
		assert(foo == 'bar', "esperava-se que foo fosse o mesmo que `bar`");

		err(function() {
			assert(foo == 'baz', "esperava-se que foo fosse o mesmo que `bar`");
		}, "esperava-se que foo fosse o mesmo que `bar`");

		err(function() {
			assert(foo == 'baz', function() {
				return "esperava-se que foo fosse o mesmo que `bar`";
			});
		}, "esperava-se que foo fosse o mesmo que `bar`");
	});

	describe("fail", function() {
		it('deve aceitar uma mensagem como terceiro argumento', function() {
			err(function() {
				assert.fail(0, 1, 'this has failed');
			}, /this has failed/);
		});

		it('deve aceitar uma mensagem como único argumento', function() {
			err(function() {
				assert.fail('this has failed');
			}, /this has failed/);
		});

		it('deve produzir uma mensagem padrão quando chamado sem argumentos', function() {
			err(function() {
				assert.fail();
			}, /assert\.fail()/);
		});
	...
	it('isTrue', function() {
		...
		}, "blah: esperava-se que false fosse true");

		err(function() {
			assert.isTrue(1);
		}, "esperava-se que 1 fosse true");

		err(function() {
			assert.isTrue('test');
		}, "esperava-se que 'test' fosse true");
	});
	...
	it('isNotTrue', function() {
		...
		}, "blah: esperava-se que true não fosse o mesmo que true");
	});
	...
	it('isOk / ok', function() {
		['isOk', 'ok'].forEach(function(isOk) {
			...
			}, "blah: esperava-se que false fosse verdadeiro");

			err(function() {
				assert[isOk](0);
			}, "esperava-se que +0 fosse verdadeiro");

			err(function() {
				assert[isOk]('');
			}, "esperava-se que '' fosse verdadeiro");
		});
	});
	...
	it('isNotOk, notOk', function() {
		['isNotOk', 'notOk'].forEach(function(isNotOk) {
			...
			}, "blah: esperava-se que true fosse falso");

			err(function() {
				assert[isNotOk](1);
			}, "esperava-se que 1 fosse falso");

			err(function() {
				assert[isNotOk]('test');
			}, "esperava-se que 'test' fosse falso");
		});
	});
	...
	it('isFalse', function() {
		...
		}, "blah: esperava-se que true fosse false");

		err(function() {
			assert.isFalse(0);
		}, "esperava-se que +0 fosse false");
	});
	...
	it('isNotFalse', function() {
		...
		}, "blah: esperava-se que false não fosse o mesmo que false");
	});
	...
	it('equal', function() {
		...
		}, "blah: esperava-se que 1 fosse o mesmo que 2");
	});
	...
	it('typeof', function() {
		...
		}, "blah: esperava-se que 5 fosse uma função");

		err(function() {
			assert.typeOf(function() {}, 'asyncfunction', 'blah');
		}, "blah: esperava-se que [function] fosse uma asyncfunction");
		...
		}, "blah: esperava-se que 5 fosse uma string");
	});
	...
	it('notTypeOf', function() {
		...
		}, "blah: esperava-se que 5 não fosse um número");

		err(function() {
			assert.notTypeOf(() => {}, 'function', 'blah');
		}, "blah: esperava-se que [function] não fosse uma função");
	});
	...
	it('instanceOf', function() {
		...
		// normalmente, `instanceof` requer que o construtor seja uma função ou
		// um objeto com um `@@hasinstance` chamável. mas em alguns navegadores
		// mais antigos, como o ie11, `instanceof` também aceita interfaces
		// relacionadas ao dom, como o `htmlelement`, apesar de serem objetos
		// não chamáveis nesses navegadores
		//
		// veja: https://github.com/chaijs/chai/issues/1000
		...
		}, "blah: a asserção `instanceof` precisa de um construtor, mas number foi recebido.");

		err(function() {
			assert.instanceOf(new Foo(), 'batman');
		}, "a asserção `instanceof` precisa de um construtor, mas string foi recebido.");

		err(function() {
			assert.instanceOf(new Foo(), {});
		}, "a asserção `instanceof` precisa de um construtor, mas object foi recebido.");

		err(function() {
			assert.instanceOf(new Foo(), true);
		}, "a asserção `instanceof` precisa de um construtor, mas boolean foi recebido.");

		err(function() {
			assert.instanceOf(new Foo(), null);
		}, "a asserção `instanceof` precisa de um construtor, mas null foi recebido.");

		err(function() {
			assert.instanceOf(new Foo(), undefined);
		}, "a asserção `instanceof` precisa de um construtor, mas undefined foi recebido.");
		...
		}, 'a asserção `instanceof` precisa de um construtor, mas function foi recebido.', true);

		err(function() {
			assert.instanceOf(new Foo(), Symbol());
		}, "a asserção `instanceof` precisa de um construtor, mas symbol foi recebido.");
		...
		}, 'esperava-se que 4 fosse uma instância de um construtor sem nome.')

		err(function() {
			assert.instanceOf(5, Foo, 'blah');
		}, "blah: esperava-se que 5 fosse uma instância de foo.");

		function CrashyObject() {};
		
		CrashyObject.prototype.inspect = function() {
			throw new Error("a função `inspect()` de `arg` foi chamada mesmo que o teste tenha passado.");
		};
	...
	it('notInstanceOf', function() {
		...
		}, 'esperava-se que o valor 4 não fosse uma instância de um construtor sem nome');

		err(function() {
			assert.notInstanceOf(new Foo(), Foo, 'blah');
		}, "blah: esperava-se que foo{} não fosse uma instância de foo");
	});
	...
	it('isObject', function() {
		...
		}, "blah: esperava-se que true fosse um objeto");

		err(function() {
			assert.isObject(Foo);
		}, "esperava-se que [function foo] fosse um objeto");

		err(function() {
			assert.isObject('foo');
		}, "esperava-se que 'foo' fosse um objeto");
	});
	...
	it('isNotObject', function() {
		...
		}, "blah: esperava-se que {} não fosse um objeto");
	});
	...
	it('notEqual', function() {
		...
		}, "blah: esperava-se que 5 não fosse o mesmo que 5");
	});
	...
	it('strictEqual', function() {
		...
		}, "blah: esperava-se que \'5\' fosse o mesmo que 5");
	});
	...
	it('deepEqual', function() {
		...
		}, "blah: esperava-se que { tea: 'chai' } fosse profundamente igualar a { tea: 'black' }");
		...
		}, "esperava-se que {} fosse profundamente igualar a {}");
	});
	...
	it('deepEqual (circular)', function() {
		...
		err(function() {
			...
			assert.deepEqual(circularObject, secondCircularObject);
		}, "esperava-se que { field: [circular] } fosse profundamente igualar a { field: [circular], …(1) }");
	});
	...
	it('notDeepEqual', function() {
		...
		}, "blah: esperava-se que { tea: \'chai\' } não fosse profundamente igualar a { tea: \'chai\' }");
	});
	...
	it('notDeepEqual (circular)', function() {
		...
		err(function() {
			...
			assert.notDeepEqual(circularObject, secondCircularObject);
		}, "esperava-se que { field: [circular] } não fosse profundamente igualar a { field: [circular] }");
	});
	...
	it('isNull', function() {
		...
		}, "blah: esperava-se que undefined fosse o mesmo que null");
	});
	...
	it('isNotNull', function() {
		...
		}, "blah: esperava-se que null to not equal null");
	});
	...
	it('isNaN', function() {
		...
		}, "blah: esperava-se que Infinity to be NaN");

		err(function() {
			assert.isNaN(undefined);
		}, "esperava-se que undefined to be NaN");

		err(function() {
			assert.isNaN({});
		}, "esperava-se que {} to be NaN");

		err(function() {
			assert.isNaN(4);
		}, "esperava-se que 4 to be NaN");
	});
	...
	it('isNotNaN', function() {
		...
		}, "blah: esperava-se que NaN not to be NaN");
	});
	...
	it('exists', function() {
		...
		}, "blah: esperava-se que undefined to exist");
	});
	...
	it('notExists', function() {
		...
		}, "blah: esperava-se que 'awesome' to not exist");
	});
	...
	it('isUndefined', function() {
		...
		}, "blah: esperava-se que null to equal undefined");
	});
	...
	it('isDefined', function() {
		...
		}, "blah: esperava-se que undefined to not equal undefined");
	});
	...
	it('isCallable', function() {
		...
		}, "blah: esperava-se que {} to be a callable function");
	});
	...
	it('isNotCallable', function() {
		...
		}, "blah: esperava-se que [Function] not to be a callable function");
	});
	...
	it('isArray', function() {
		...
		err(function() {
			assert.isArray({}, 'blah');
		}, "blah: esperava-se que {} to be an array");
	});
	...
	it('isNotArray', function() {
		...
		}, "blah: esperava-se que [] not to be an array");

		err(function() {
			assert.isNotArray(new Array());
		}, "esperava-se que [] not to be an array");
	});
	...
	it('isString', function() {
		...
		}, "blah: esperava-se que 1 to be a string");
	});
	...
	it('isNotString', function() {
		...
		}, "blah: esperava-se que 'hello' not to be a string");
	});
	...
	it('isNumber', function() {
		...
		}, "blah: esperava-se que \'1\' to be a number");
	});
	...
	it('isNotNumber', function() {
		...
		}, "blah: esperava-se que 4 not to be a number");
	});
	...
	it('isNumeric', function() {
		...
		}, "blah: esperava-se que \'1\' to be numeric");
	});
	...
	it('isNotNumeric', function() {
		...
		}, "blah: esperava-se que 4 to not be numeric");
	});
	...
	it('isFinite', function() {
		...
		}, "blah: esperava-se que NaN to be a finite number");

		err(function() {
			assert.isFinite(Infinity);
		}, "esperava-se que Infinity to be a finite number");

		err(function() {
			assert.isFinite('foo');
		}, "esperava-se que \'foo\' to be a finite number");

		err(function() {
			assert.isFinite([]);
		}, "esperava-se que [] to be a finite number");
	...
	it('isBoolean', function() {
		...
		}, "blah: esperava-se que \'1\' fosse um boolean");
	});
	...
	it('isNotBoolean', function() {
		...
		}, "blah: esperava-se que true não fosse um boolean");

		err(function() {
			assert.isNotBoolean(false);
		}, "esperava-se que false não fosse um boolean");
	});
	...
	it('include', function() {
		...
		}, "blah: esperava-se que \'foobar\' incluísse \'baz\'");
		...
		}, "esperava-se que [ { a: 1 }, { b: 2 } ] incluísse { a: 1 }");
	...
	it('keys(array|Object|arguments)', function() {
		...
		assert.doesNotHaveAllKeys(testMap, [aKey, {
			iDoNot: 'exist'
		}]);
		...
		assert.doesNotHaveAnyKeys(testMap, ['thisDoesNotExist', 'thisToo', {
			iDoNot: 'exist'
		}]);
		...
		// certificar-se de que as afirmações acima utilizem o princípio da igualdade estrita
		assert.doesNotHaveAnyKeys(testMap, {
			thisIs: 'anExampleObject'
		});
		...
		err(function() {
			assert.containsAllKeys(testMap, [{
				thisIs: 'anExampleObject'
			}]);
		});

		// testes para as variações profundas da asserção de chaves
		...
		assert.doesNotHaveAllKeys(testSet, [aKey, {
			iDoNot: 'exist'
		}]);
		...
		assert.doesNotHaveAnyKeys(testSet, ['thisDoesNotExist', 'thisToo', {
			iDoNot: 'exist'
		}]);
		...
		assert.doesNotHaveAnyKeys(testSet, [20, 1, {
			iDoNot: 'exist'
		}]);

		assert.doesNotHaveAllKeys(testSet, ['thisDoesNotExist', 'thisToo', {
			iDoNot: 'exist'
		}]);
		...
		err(function() {
			assert.containsAllKeys(testSet, [{
				thisIs: 'anExampleObject'
			}]);
		});
		...
		// repete os testes anteriores com o objeto como argumento
		err(function() {
			assert.hasAllKeys({
				foo: 1
			}, {
	...
	it('deepPropertyVal', function() {
		...
		assert.deepPropertyVal(obj, 'a', {
			b: 1
		});
		
		assert.notDeepPropertyVal(obj, 'a', {
			b: 7
		});
		...
		assert.notDeepPropertyVal(obj, 'z', {
			b: 1
		});
	...
	it('deepOwnPropertyVal', function() {
		...
		assert.deepOwnPropertyVal(obj, 'a', {
			b: 1
		});
		
		assert.notDeepOwnPropertyVal(obj, 'a', {
			z: 1
		});
		...
		err(function() {
			assert.deepOwnPropertyVal(obj, 'z', {
				b: 1
			}, 'blah');
	...
	it('deepNestedPropertyVal', function() {
		...
		assert.notDeepNestedPropertyVal(obj, 'a.z', {
			c: 1
		});
	...
	it('throws / throw / Throw', function() {
		...
		['throws', 'throw', 'Throw'].forEach(function(throws) {
			...
			assert(thrownErr instanceof Error, 'assert.' + throws + ' retorna um erro');
			assert(thrownErr.message === 'foo', 'assert.' + throws + ' retorna uma mensagem de erro');
	...
	it('doesNotThrow', function() {
		...
		assert.doesNotThrow(function() {
			throw new Error('isso é uma mensagem');
		}, TypeError);
	...
	it('ifError', function() {
		...
		err(function() {
			var err = new Error('isso é uma mensagem de erro');
	...
	it('operator', function() {
		// para testar valores indefinidos e nulos com == e ===
	...
	it('isExtensible / extensible', function() {
		['isExtensible', 'extensible'].forEach(function(isExtensible) {
			...
			err(function() {
				// isextensible não deve suprimir erros lançados em traps de proxy
	...
	it('showdiff true with actual and expected args', function() {
		try {
			new chai.Assertion().assert(
				'one' === 'two', 'esperava-se que #{this} fosse o mesmo que #{exp}', 'esperava-se que #{this} não fosse o mesmo que #{act}', 'one', 'two'
			);
		} catch (e) {
	...
	it('showdiff falso sem os esperados e atuais', function() {
		try {
			new chai.Assertion().assert(
				'one' === 'two', 'esperava-se #{this} fosse o mesmo que #{exp}', 'esperava-se #{this} não fosse o mesmo que #{act}', 'one', 'two', false
			);
		} catch (e) {
