...
##############################################################
# funções úteis comuns para scripts de compilação do android #
##############################################################

if [ -z "$PYTORCH_DIR" ]; then
    echo "PYTORCH_DIR não confiurado!"
...
check_android_sdk() {
    if [ -z "$ANDROID_HOME" ]; then
        echo "ANDROID_HOME não configurado; configure-o no diretório sdk do android"

        exit 1
    fi

    if [ ! -d "$ANDROID_HOME" ]; then
        echo "ANDROID_HOME não é um diretório; você instalou por meio de $ANDROID_HOME?"
...
parse_abis_list() {
    # sincronizar com https://github.com/pytorch/pytorch/blob/0ca0e02685a9d033ac4f04e2fa5c8ba6dbc5ae50/android/gradle.properties#L1
...
build_android() {
    ...
    # esses diretórios contém apenas links simbólicos
