...
if (typeof Promise === 'undefined') {
    console.log('nenhuma promise global encontrada, pulando testes de promisify');
...

{
    ...
    promise.catch(common.mustCall(function (error) {
        assert(error.message.indexOf('enoent: nenhum arquivo ou diretório desse tipo, stat') !== -1);
    }));
}
...
// argumentos de promisify desabilitados, há uma api core interna
// que não é mais utilizada em nenhum lugar e esse pacote não
// a implementa
if (false) {
    var firstValue = 5;
    var secondValue = 17;
...

{
    ...
    assert.notStrictEqual(Object.getPrototypeOf(promisify(fn4)), Function.prototype);
}
...

{
    ...
    thrower(1, 2, 3)
        ...
        .then(assert.fail, function (e) {
            assert.strictEqual(e, errToThrow);
        });
}

{
    ...
    var a = promisify(function (cb) {
        cb(err)
    })();

    var b = promisify(function () {
        throw err;
    })();
