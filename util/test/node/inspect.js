...
// testar a implementação interna isdate
...
// teste para descritores de propriedade
...
// exceções devem printar a mensagem de erro, e não '{}'
...
try {
    ...
    assert.equal(util.inspect(e), '[ReferenceError: undef não está definido]');
}
...
/* não tenho certeza do porquê isso não funciona
...
// bug com a função de inspeção fornecida pelo usuário retorna uma string não especificada
assert.doesNotThrow(function() {
    util.inspect([{
        inspect: function() {
            return 123;
...
// util.inspect.styles e util.inspect.colors
function test_color_style(style, input, implicit) {
    ...
    assert.equal(with_color, expect, 'cor de util.inspect para estilo '+style);
}
...
test_color_style('string', 'string de teste');
test_color_style('date', new Date);
test_color_style('regexp', /regexp/);

// um objeto com "hasownproperty" reescrito não deve disparar
assert.doesNotThrow(function() {
    util.inspect({
        hasOwnProperty: null
    });
...
// nova api, aceita um objeto "options"
var subject = {
    foo: 'bar',
    hello: 31,
...
// a opção "custominspect" pode habilitar/desabilitar de chamar pelo inspect() em objetos
subject = {
    inspect: function() {
        return 123;
    }
...
// funções inspect() customizadas devem ser capazes de retornar outros objetos
...
// util.inspect com opção "colors" deve produzir tanto com linhas quanto sem
...
test_lines({
    ...
    baz: 35,
    b: {a: 35},
    very_long_key: 'valor_bem_longo',
    even_longer_key: ['com valor ainda maior no array']
});