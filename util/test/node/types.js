// atualmente sincronizado com o node.js test/parallel/test-util-types.js
...
var _require = require('../../'), types = _require.types;
var vm = require('vm');

var Buffer = require('safe-buffer').Buffer
var objectEntries = require('object.entries');

// "polyfill" o deepstrictequal no node.js 0.12 e abaixo
...
// [browserify] não se pode testar isso na área de usuário
//
...
for (var _i = 0, _arr = [
    ...
    try {
        ...
        console.log('pulando tipo não suportado:', getValue);
    ...
    assert(method in types, "faltando ".concat(method, " for ").concat(inspect(_value)));
    assert(types[method](_value), "deseja-se ".concat(inspect(_value), " para coincidir ").concat(method));

    for (var _i3 = 0, _Object$keys2 = Object.keys(types); _i3 < _Object$keys2.length; _i3++) {
        var key = _Object$keys2[_i3];

        if ((types.isArrayBufferView(_value) || types.isAnyArrayBuffer(_value)) && key.indexOf('Array') > -1 || key === 'isBoxedPrimitive') {
            continue;
        }
...
// checar primitivos boxed
console.log('testando', 'isBoxedPrimitive');
...
var isBuggyFirefox = typeof navigator !== 'undefined' && /Firefox\/\d+/.test(navigator.userAgent) && parseInt(navigator.userAgent.split('Firefox/')[1], 10) < 66;
...
if (SymbolToStringTagSupported && !isBuggyFirefox) {
    [
        ...
        console.log('testando arrays com tipos falsos', method);
...
if (isBuggyFirefox) {
    console.log('pulando testes de arrays falsas porque elas não funcionam no ff')
}

// o antigo node.js tinha uma implementação de buffer totalmente personalizada, os mais novos são baseados em arraybuffer
//
// isso é importante para os testes de arraybuffer e array tipado
...

{
    ...
    Object.keys(expected).forEach(function (testedFunc) {
        console.log('testando valores para:', testedFunc);
        ...
        assert.deepStrictEqual(
            yup, expected[testedFunc].filter(function (a) { return typeof a !== 'undefined'; })
        );
    });
}
