...
async function runBenchmark(options, implementations) {
    ...
    for (const { name, implementation } of Array.isArray(implementations) ? implementations : Object.entries(implementations).map(([name, implementation]) => ({ name, implementation }))) {
        bench.add(name, () => {
            try {
