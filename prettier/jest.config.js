...
// desabilitado https://github.com/nicolo-ribaudo/jest-light-runner/pull/13
...
// ao depurar testes de produção, esta opção permite ignorar a
...
if (INSTALL_PACKAGE || (isProduction && !TEST_STANDALONE && !SKIP_PRODUCTION_INSTALL)) {
...
if (isProduction) {
    // apenas rodar teste unitário para desenvolvimento

    testPathIgnorePatterns.push("<rootDir>/tests/unit/");
} else {
    // apenas testar bundles para produção

    testPathIgnorePatterns.push("<rootDir>/tests/integration/__tests__/bundle.js");
}

if (nodejsMajorVersion <= 16) {
    // utilizar atributos de import

    testPathIgnorePatterns.push("<rootDir>/tests/integration/__tests__/help-options.js");
}
...
if (nodejsMajorVersion <= 14) {
    testPathIgnorePatterns.push(
        ...
        // `@prettier/cli` utiliza `node:stream/consumers`, não disponível no node.js v14
        "<rootDir>/tests/integration/__tests__/experimental-cli.js",
        
        // falha no node.js v14
