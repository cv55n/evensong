...
/**
 * encontrar o arquivo de cache padrão:
 * 
...
async function findCacheFileFromOption(cacheLocation) {
    ...
    if (stat) {
        if (stat.isDirectory()) {
            throw new Error(`--cache-location resolvido '${cacheFile}' é um diretório`);
        }

        const data = await fs.readFile(cacheFile, "utf8");

        if (!isJson(data)) {
            throw new Error(`'${cacheFile}' não é um arquivo json válido`);
        }
    }
