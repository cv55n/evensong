    ...
    try {
        if (context.argv.config === false) {
            context.logger.debug(
                "opção '--no-config' encontrada, pulando o arquivo de configuração."
            );
        ...
        context.logger.debug(
            context.argv.config
                ? `carregando arquivo de configuração em '${context.argv.config}'`
                : `resolvendo configuração em '${filePath}'`
        );
        ...
        context.logger.debug("opções `" + JSON.stringify(options) + "` carregadas");

        return options;
    } catch (error) {
        context.logger.error(
            `configuração ${filePath ? ` para o arquivo "${filePath}"` : ""} inválida:\n` + error.message
        );
...
async function getOptionsForFile(context, filepath) {
    ...
    const appliedOptions = {
        ...
        ...applyConfigPrecedence(
            context,
            options && normalizeApiOptions(options, context.supportOptions, {
