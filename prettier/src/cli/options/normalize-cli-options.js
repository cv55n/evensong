    ...
    pair: ({ key, value }) =>
        value === false
            ...
            : value === true
                ? descriptor.key(key)
                : value === ""
                    ? `${descriptor.key(key)} sem um argumento`
    ...
    preprocess(value, utils) {
        if (typeof value === "string" && value.length > 0 && !this.#flags.includes(value)) {
            ...
            if (suggestion) {
                utils.logger.warn(
                    [
                        `flag ${picocolors.yellow(utils.descriptor.value(value))} desconhecida`,
                        `você quis dizer ${picocolors.blue(utils.descriptor.value(suggestion))}?`
                    ].join(" ")
                );
