...
function getOptionDefaultValue(context, optionName) {
    ...
    return (
        formatOptionsHiddenDefaults[optionCamelName] ?? context.supportOptions.find(
...
function createChoiceUsages(choices, margin, indentation) {
    ...
    return activeChoices.map((choice) => indent(
        createOptionUsageRow(choice.value, choice.description, threshold), indentation
    ));
}
...
function createOptionUsage(context, option, threshold) {
    ...
    return createOptionUsageRow(header, `${option.description}${
...
function getOptionsWithOpposites(options) {
    // adicionar --no-foo depois de --foo
...
function createUsage(context) {
    ...
    const options = getOptionsWithOpposites(sortedOptions).filter(
        // remover opções desnecessária (ex: `semi`, `color`, etc.)
        // que só é utilizada para --help <flag>
...
function createPluginDefaults(pluginDefaults) {
    ...
    const defaults = Object.entries(pluginDefaults)
        ...
        .map(([pluginDefaults, value]) => `* ${plugin}: ${createDefaultValueDisplay(value)}`)
        .join("\n");
