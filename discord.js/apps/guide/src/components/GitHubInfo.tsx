...
async function getRepoStarsAndForks(
    ...
    if (!response.ok) {
        ...
        throw new Error(`falha ao capturar os dados do repositório: ${message}`);
    }
...
/**
 * converte um número em uma string legível para humanos com
 * o sufixo k para representar milhares
 * 
...
function humanizeNumber(num: number): string {
    ...
    if (num < 100_000) {
        // para números entre 1000 e 99999, mostrar com uma casa decimal (ex: 1.5k)
        const value = (num / 1_000).toFixed(1);

        // remoção do ".0" final, caso existente
    ...
    if (num < 1_000_000) {
        // para números entre 10000 e 999999, mostrar como todo k (ex: 10k, 999k)
        return `${Math.floor(num / 1_000)}k`;
    }

    // para 1000000 para cima, apenas retornar o número
    return num.toString();
}